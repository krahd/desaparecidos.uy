...
- Process videos may briefly reveal a contributing approved place image at full opacity before non-contributing pixels fade away. For `people`, reveal only the reviewed face region used for fragment extraction, never the surrounding contemporary photograph. Never reveal rejected or non-contributing candidate images, and record the reveal policy and grid/match layout in the sidecar.
...
- Computer-vision gating must require real detected faces for `targets` and `people`; do not use fallback face boxes. `places` gating must prefer photo-like non-face scene material and reject obvious graphics, logos/posters, prominent faces, and noise-like textures. CV never auto-approves crawler manifest rows (`targets`, `places`, `people`) and never identifies people.
- Seguimos buscando traversal frames are the explicit, artist-decided exception to manual-only approval (decision recorded 2026-07-08): acquisition may auto-approve frames that pass the CV place gate when `auto_approve` is requested (the GUI default and the one-shot autonomous flow). Auto-approved frames carry `review_policy: "auto-cv-accepted"`, remain fully reversible in frame review, and a manual review decision always survives re-acquisition. CV-rejected and duplicate frames are never auto-approved. Traversal outputs remain `internal_unreviewed` regardless of approval policy.
