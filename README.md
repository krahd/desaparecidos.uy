...
The artist can choose fixed regions, largest-first regions that preserve accepted material, or largest-first regions with later refinement. Variable square and rectangular regions range from the minimum size (default 96 px) to the maximum (384 px). Search prioritises the largest uncovered area on its multiscale lattice, then improves accepted regions. Refinements must improve the existing match by the selected margin. The structural similarity threshold defaults to 0.72, minimum structure to 0.035, and refinement margin to 0.04; these are artistic controls, not probabilities or semantic recognition scores. Broad structure (default) compares an 8×8 arrangement of light/dark areas and signed gradients; fine structure retains the earlier 16×16 comparison. The optional region-exposure control adjusts an already accepted crop with one brightness/contrast transform. It adds no target pixels, cannot make a rejected crop eligible, and records the transform in the sidecar. Source exposure remains the default. **Use broad portrait settings** on the artist screen applies the 192–768 px, 0.60-threshold, exposure-adjusted configuration used for the complete internal portrait study; completion still depends on the available structures. Sidecars record every encounter, accepted region, skipped frame, refinement, score and realised coverage.

All three works share a monochrome 1920×1080 / 24 fps video default: **search left, reconstruction right → reconstructed image → person details → closing text**. Continuous playback advances the city sequence at six source frames per second (0.16 seconds rounded to four output frames). Each accepted crop travels to its recorded destination over 0.75 seconds while the traversal continues. The final transfer lands before the reconstructed-image hold. Actual duration follows the number of encounters and their cadence; a long duration request does not stretch a short walk. The artist can select pause-on-contribution playback to retain the older minimum-duration behaviour. Closing defaults are four seconds for the image, three for details, two for text, with one-second fades. Artist controls include playback mode, traversal/transfer timing, frame rate, resolution, match marks and closing text. `video_presentation` records exact encounter, launch, arrival and phase timing. Overlay, alternating and stacked presentations remain artist overrides; the default is side by side.
