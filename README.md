...
The artist can choose fixed regions, largest-first regions that preserve accepted material, or largest-first regions with later refinement. Variable square and rectangular regions range from the minimum size (default 96 px) to the maximum (384 px). Search prefers the largest qualifying region on its multiscale lattice. Refinements must improve the existing match by the selected margin. The structural similarity threshold defaults to 0.72, minimum structure to 0.035, and refinement margin to 0.04; these are artistic controls, not probabilities or semantic recognition scores. Sidecars record every encounter, accepted region, skipped frame, refinement, score and realised coverage.

All three works share a monochrome 1920×1080 / 24 fps video default: **search left, reconstruction right → reconstructed image → person details → closing text**. Search encounters with a contribution last at least 2.5 seconds by default, showing the source before the region appears; unmatched traversal frames have a 0.18-second minimum. Closing defaults are four seconds for the image, three for details, two for text, with one-second fades. The artist screen exposes these durations, frame rate, resolution, match marks and closing text. A 60-second request uses 46 seconds of search and 14 seconds of closing when all encounters fit; the renderer extends the duration when necessary to show every encounter and preserve the minimum holds. It never drops source encounters to meet a short duration. `video_presentation` records the requested and actual duration and exact phase counts. Overlay, alternating and stacked presentations remain deliberate artist overrides; the default is side by side.
