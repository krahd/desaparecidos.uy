...
- monochrome still PNGs and browser-playable H.264 videos with search at left, reconstruction at right, followed by the assembled image, person details and closing text;
...
The **Images** workspace crawls every page once for both `places` and `people`, downloads each candidate once, applies the two CV policies independently, and appends accepted candidates to separate pending manifests. Exact and perceptual deduplication filters resized and compressed versions before they become new rows. Manual approval remains mandatory. Pending, rejected, or approved rows can be sent to the other collection: the destination copy becomes approved and preserves provenance, while the source row becomes rejected; transfer to `people` still requires a detected eligible face region. `targets` remain the disappeared-person portrait corpus. The `/api/download` route and CLI download command still exist, but the primary GUI no longer shows a download panel. Synthetic demo fixture controls are in the compact Utilities modal.
...
Frame approval supports two policies. Manual review remains the default for drawn/imported routes. Acquisition may instead auto-approve frames that pass the local CV place gate (`auto_approve`, the GUI default); these decisions remain reversible and never overwrite manual review. Rendering follows capture order and uses direct jump cuts across sequence gaps.

**Seguimos buscando** compares normalised grayscale structure and signed directional edges. Each approved frame can contribute **at most one region, from that current frame**. A frame without a sufficiently strong structural match contributes nothing and the walk continues. Multiple coherent walks within a saved traversal accumulate into the same portrait in single-target mode; ordered multi-target mode divides the traversal into contiguous segments. No fill quota or reuse of an earlier frame forces completion. Empty and partial reconstructions are valid.

The artist can choose fixed regions, largest-first regions that preserve accepted material, or largest-first regions with later refinement. Variable square and rectangular regions range from the minimum size (default 96 px) to the maximum (384 px). Search prioritises the largest uncovered area on its multiscale lattice, then improves accepted regions. Refinements must improve the existing match by the selected margin. The structural similarity threshold defaults to 0.82, minimum structure to 0.035, and refinement margin to 0.04; these are artistic controls, not probabilities or semantic recognition scores. Broad structure (default) compares an 8×8 arrangement of light/dark areas and signed gradients; fine structure retains the earlier 16×16 comparison. The optional region-exposure control adjusts an already accepted crop with one brightness/contrast transform. It adds no target pixels, cannot make a rejected crop eligible, and records the transform in the sidecar. Source exposure remains the default. **Use complete portrait settings** retains 192–768 px regions and exposure adjustment, with the verified completion setting of 0.72 acceptance (above the original 0.60 study). The general structural default remains 0.82 for stricter experiments. Completion is now required by default: matching concentrates on uncovered regions and searches wider source crops on a dense analysis grid with native-resolution verification, without shrinking destination regions. Single-target Uruguay searches automatically acquire further bounded batches through the existing CV approval policy. The video closes only at full coverage; a network/resource limit saves progress and reports a paused search instead of producing an incomplete final video. Resume the same saved traversal to continue. Drawn routes and target sequences require additional approved material supplied to their existing traversal. `--allow-incomplete` explicitly enables study outputs. After reaching full coverage, the optional global similarity goal permits bounded refinement within the nominal search-playback budget. Once that budget is spent, full coverage ends the search even if further refinement is possible; missing regions always extend it.

All three works share a monochrome 1920×1080 / 24 fps video default: **search left, reconstruction right → black → reconstructed image → black → person details → black → closing text → black → https://desaparecidos.uy → black**. Traversal playback starts at three source frames per second. A soft 300-second search budget accelerates unmatched encounters down to one encoded frame where needed; every encounter and contribution remains in chronological order. Contribution spacing is applied to playback rather than discarding useful matches. Completion and readable transfers take precedence over this duration target. Fragment works retain their previous cadence. Each accepted crop travels to its recorded destination over 0.75 seconds while the traversal continues. The final transfer lands before search fades to black and the reconstructed image fades in. Catalogue names are displayed as given names followed by surnames, without the comma; stored metadata is unchanged. The website fades in after the title fades out, holds for the text-hold duration, then fades to black. Actual duration follows the recorded per-encounter holds, contribution spacing and transfer landings; the playback budget is not a search cutoff. The artist can select pause-on-contribution playback to retain the older minimum-duration behaviour. Closing defaults are four seconds for the image, three for details, two for text, with one-second fades. Artist controls include playback mode, traversal/transfer timing, frame rate, resolution, match marks and closing text. `video_presentation` records exact encounter, launch, arrival and phase timing. Overlay, alternating and stacked presentations remain artist overrides; the default is side by side.
...
- `Video source layout`: approved source-region reveal (`grid`) or contributing fragments only (`match`);
- `reuse_limit`, `output_width`, seed, still generation, and video generation;
- monochrome output for every work, including the artist interface, previews and map; colour requests are rejected.

The compatibility renderer may reveal a contributing approved place image or the reviewed detected people face region. Its fragment-only option hides the full source region. Canonical fragment-work videos retain fragment-only source display. Contemporary surroundings and rejected sources are excluded. The traversal video also shows approved unmatched frames as the ongoing search, without adding them to the reconstruction. Every final reconstructed image contains only contributed fragments; missing regions stay black and the historical portrait is never blended in to simulate completion.
...
python -m desaparecidos run-traversal --traversal route-ID --target-id person-ID --targets data/manifests/targets.csv --composition split --duration 60 --fps 24 --output-width 1920
