...
- deterministic, vectorised fragment matching using a deliberately modest colour/contrast/edge descriptor and explicit per-source contribution caps;
...
Generated outputs, raw downloads, crawl caches, crawl trails, rejected candidates, temporary processed copies, and review manifests are intentionally ignored by git. The curated portrait collection in `doc/fotos-desaparecidos/`, the canonical target store at `data/persons/disappeared.json`, reviewed metadata overrides in `data/persons/metadata-overrides.csv`, and reviewed selected portrait derivatives under `assets/targets/disappeared/selected/` are trackable project corpus files.
...
The **Targets** screen edits `data/persons/disappeared.json`. It lists disappeared-person records, filters records with missing information or portrait-review needs, edits full name, birth/disappearance/death/remains fields, stores source/provenance notes, downloads explicit portrait candidates by URL, processes the selected candidate into a white-border-trimmed 3:4 portrait, and exports the derived `targets.csv` manifest. Online acquisition is controlled: trusted sources in `data/sources.json` are preferred, general web results are candidate evidence only, raw-only candidates are not auto-selected, and arbitrary web images are never made authoritative without review.
...
- `Max tiles per source`, wired to `max_contribution_per_source` (defaults to `1`; set `0` for explicit unlimited use);
...
python -m desaparecidos run-stage1 --targets data/manifests/targets.csv --sources data/manifests/places.csv --output outputs/stage1 --seed 17 --max-contribution-per-source 1 --search-scan-frames-per-candidate 2 --search-scan-max-candidates 120
...
python scripts/audit_target_corpus.py
python scripts/audit_target_corpus.py --portrait-review-only
python scripts/apply_person_metadata_overrides.py --write
python scripts/suggest_local_portrait_matches.py --min-score 0.85
```

The Sitios de Memoria importer and portrait/metadata override tooling remain available for corpus preparation. The importer selects a portrait only from a person page's `field-fotografia` container, recording `portrait_status` as `ok` or `missing` rather than importing work posters or materials as a face. It stores labelled death dates as `date_of_death` instead of mixing them into detention fields. It writes the canonical person store at `data/persons/disappeared.json`, raw downloads under ignored `assets/targets/disappeared/raw/`, selected 3:4 derivatives under `assets/targets/disappeared/selected/`, and the derived `data/manifests/targets.csv`. Each person record carries a `field_sources` map noting which source (see the tracked `data/sources.json` registry) is authoritative for each field and for the selected portrait; reviewed CSV corrections in `data/persons/metadata-overrides.csv` add per-field `field_source_refs` through `scripts/apply_person_metadata_overrides.py`. `scripts/apply_portrait_overrides.py` swaps in a portrait from a better source (for example Parque de la Memoria for Argentine cases) and records that provenance. `scripts/audit_target_corpus.py` reports selected-portrait coverage, candidate counts, portrait-review needs, missing fields, portrait source counts, and unresolved person records. `scripts/suggest_local_portrait_matches.py` compares the older `local-targets.csv` / `doc/fotos-desaparecidos` portrait corpus to the canonical person store and can append strong matches as review-only candidates; it never selects or exports them automatically.
...
- The canonical target corpus is `data/persons/disappeared.json`; target manifests are derived export files for generation compatibility. For killed cases with no separate disappearance date/place, export uses the reviewed death date/place rather than fabricating disappearance data.
...
- Local `doc/fotos-desaparecidos` filename matches are review-only portrait candidates. They can help choose better-quality selected portraits, but they are not identity proof and must be confirmed before processing/selection.
...
- Stage 1 matching uses non-overlapping 24-pixel tiles by default. Each fragment is described by mean RGB, luminance contrast, and horizontal/vertical edge energy, then matched by deterministic L2 nearest-neighbour search. This modest matcher keeps seams, mismatches, and fragment boundaries visible rather than smoothing the output into a synthetic restoration.
- `reuse_limit` is enforced per extracted source fragment. `max_contribution_per_source` caps how many output tiles any single source image fills; it defaults to `1`, `0` remains explicit unlimited use, and infeasible caps fail before generation.
