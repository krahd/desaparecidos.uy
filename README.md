...
The artist can choose fixed regions, largest-first regions that preserve accepted material, or largest-first regions with later refinement. Variable square and rectangular regions range from the minimum size (default 96 px) to the maximum (384 px). Search prioritises the largest uncovered area on its multiscale lattice, then improves accepted regions. Refinements must improve the existing match by the selected margin. The structural similarity threshold defaults to 0.82, minimum structure to 0.035, and refinement margin to 0.04; these are artistic controls, not probabilities or semantic recognition scores. Broad structure (default) compares an 8×8 arrangement of light/dark areas and signed gradients; fine structure retains the earlier 16×16 comparison. The optional region-exposure control adjusts an already accepted crop with one brightness/contrast transform. It adds no target pixels, cannot make a rejected crop eligible, and records the transform in the sidecar. Source exposure remains the default. **Use longer, stricter portrait search** keeps the study's 192–768 px regions and exposure adjustment, raises acceptance from 0.60 to 0.82, advances at three source frames per second, and spaces contributions by at least six encounters. Search ends only after full coverage and the reconstruction similarity goal (default 0.95), or exhaustion of approved frames. Similarity is the lower of whole-image structural correlation and `1 − mean absolute grayscale error / 255`; it is a heuristic, not a fidelity guarantee. Sidecars distinguish `quality-target-reached` from `approved-frames-exhausted`. Add approved traversal material if the goal is unmet; the renderer does not acquire new imagery or shrink regions to force completion. Spacing limits early bursts but cannot guarantee equal contributions from unequal source material.

All three works share a monochrome 1920×1080 / 24 fps video default: **search left, reconstruction right → black → reconstructed image → black → person details → black → closing text → black → https://desaparecidos.uy → black**. Traversal playback advances at three source frames per second (0.33 seconds rounded to eight output frames at 24 fps); fragment works retain 0.16 seconds. Each accepted crop travels to its recorded destination over 0.75 seconds while the traversal continues. The final transfer lands before search fades to black and the reconstructed image fades in. Catalogue names are displayed as given names followed by surnames, without the comma; stored metadata is unchanged. The website fades in after the title fades out, holds for the text-hold duration, then fades to black. Actual duration follows the number of encounters and their cadence; a long duration request does not stretch a short walk. The artist can select pause-on-contribution playback to retain the older minimum-duration behaviour. Closing defaults are four seconds for the image, three for details, two for text, with one-second fades. Artist controls include playback mode, traversal/transfer timing, frame rate, resolution, match marks and closing text. `video_presentation` records exact encounter, launch, arrival and phase timing. Overlay, alternating and stacked presentations remain artist overrides; the default is side by side.
