...
- 2026-06-17 23:24 GMT-3 follow-up: `.venv/bin/python -m pytest -q` (48 tests, +3 for review-row delete) and `npm --prefix frontend run build` passed. Rendered the real review CSS in headless Chromium to confirm correct thumbnail aspect ratio. A two-session crawl smoke confirmed the manifest, content store, and `cache.sqlite` persist on disk, that all referenced files exist on a fresh read, and that approve, reject, and delete behave correctly (the deleted row's cached file remained).
