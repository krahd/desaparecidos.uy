...
Last updated: 2026-08-02 02:39 GMT-3
...
All GUI/API, legacy CLI and canonical artwork generation paths now render stills and video frames in grayscale by default. Colour output is an explicit opt-in recorded as `colour_output` in sidecar settings and exposed through the GUI and `--colour` CLI flags. Matching continues to use the original colour-aware descriptors before output conversion.
...
- crawler perceptual deduplication uses a 10-bit Hamming-distance tolerance to filter more resized and compressed variants while retaining exact SHA-256 deduplication;
...
- pending, rejected, or approved place/people rows can be sent to the other collection with provenance preserved; the destination becomes approved, the source is rejected for audit/reversal, and transfer into People requires the existing real-face CV gate and a stored face region;
...
Local verification on 2 August 2026 after the crawler, image-review, cross-collection transfer and grayscale-output changes:

- `python -m compileall -q src tests` — passed;
- `python -m pytest -q` — 168 passed with five existing deprecation warnings;
