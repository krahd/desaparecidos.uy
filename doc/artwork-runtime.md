...
### Traversal quality and pacing

`--contribution-interval 6` requires at least six encounters before the first contribution and between subsequent contributions. Intervening approved frames remain visible search encounters, with `contribution-spacing` recorded as their skip reason. No earlier frame is deferred or reused. Set the interval to 1 for unrestricted encounter matching.

`--search-similarity 0.95` stops at full pixel coverage and the requested whole-image similarity; 0 disables early stopping. Similarity is the minimum of normalised structural correlation at the selected structure scale and `1 - mean absolute grayscale error / 255`, measured from the assembled source image. Exhausting the finite approved traversal is explicitly recorded as `approved-frames-exhausted`, even at full coverage if quality remains below the goal. The renderer does not automatically discover/acquire additional material. Minimum/maximum region sizes never decrease to reach the goal.

Presentation schema 3.0 adds search fade-out, reconstruction fade-in and a final `https://desaparecidos.uy` fade-in/hold/fade-out after the title. Website hold uses `text_hold_seconds`. Comma-separated catalogue names become given names followed by surnames on the person card. These closing changes apply to the shared compositor across all three works.
