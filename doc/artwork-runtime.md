...
`--playback-mode` (`continuous` or `hold`), `--contribution-seconds` (0.75), `--scan-seconds` (0.33 for traversal, 0.16 for fragment works), `--final-hold-seconds` (4), `--details-hold-seconds` (3), `--text-hold-seconds` (2), `--fade-seconds` (1), `--closing-text`, `--hide-match-marks`, `--fps`, `--duration` and `--output-width` expose the presentation choices in both CLIs. Equivalent fields are available on the localhost artist screens/API. `--split-orientation stacked` is an optional override; the default is `side-by-side`. The deprecated colour flags are rejected.

Continuous playback advances every source encounter at the scan cadence (three per second for traversal at the defaults), while crops travel to their destinations over the contribution duration. Several transfers may be in flight, but each input contributes at most one region and deposits remain in encounter order. The last crop lands before the closing phases. Duration follows these events rather than stretching the walk to the requested length. `hold` playback retains the minimum-duration behaviour and pauses on contributions. `video_presentation` records actual duration, phase allocations and per-placement launch/arrival frames, relative to each target segment.
...
The default `refine` method searches square and rectangular regions from `--fragment-size 96` through `--max-region-size 384`, prioritising the largest uncovered area on a bounded multiscale lattice before refining occupied areas. `--reconstruction-mode largest-first` preserves accepted regions; `fixed` uses only the minimum square size. Each source frame can supply at most one region, only at its own encounter. There is no found-fragment pool, per-frame filling quota or forced completion. The target persists across the coherent walks within a traversal. Empty areas stay black.

Matching compares normalised grayscale organisation and signed directional gradients. `--structure-scale broad` (default) compares 8×8 samples to emphasise broad graphic relationships; `fine` retains the earlier 16×16 comparison. `--tone-mode source` preserves source exposure. Optional `match-region` adjusts only an accepted source crop with one affine brightness/contrast transform, with gain bounded to 0.25–4.0. It does not copy target pixels, relax the structural threshold or fill rejected areas. The decision records gain, offset and clipped fraction. Flat regions are ineligible. The controls are `--structure-threshold 0.82`, `--min-structure 0.035` and `--refinement-margin 0.04`. Scores express this heuristic correlation, not semantic identification or calibrated confidence. Later replacements must improve both the current rectangle's measured structure and the recorded quality of every covered area by the margin. Refinement can replace a whole earlier region or a smaller part. Histories retain all contributions in encounter order, including superseded material; `source_usage` counts historical contributions.
...
### Traversal completion and pacing

Completion is required by default. The matcher preserves every useful current-frame match and prioritises uncovered regions, using additional source scales and a bounded dense descriptor bank without shrinking the configured destination regions. Dense candidates and locally refined coarse candidates are checked against their native-resolution source crops before acceptance. The per-region structural threshold remains mandatory. At full pixel coverage, `--search-similarity` permits bounded refinement until the goal is met or the nominal encounter budget (`search_budget_seconds / scan_seconds`) is spent. Beyond that budget, missing coverage continues the search but a complete portrait stops. Exhausting available frames at full coverage also stops without acquiring more imagery. Set the similarity goal to 0 for immediate stopping at full coverage. The artist preset uses 192–768 px regions and 0.72 acceptance; the general default remains 0.82.

Single-target saved Uruguay traversals extend through up to `--max-search-batches 8` additional acquisition batches per invocation, each capped at 600 frames across up to 12 walks. Existing CV approval, manual decisions and exact deduplication remain in force. Appended frames retain original traversal identity and source metadata. Progress is saved under `completion_search` in the ignored route manifest. Network failure or batch exhaustion pauses the job without finalising an incomplete video; render the same traversal again to continue from its saved material. Drawn routes and multi-target sequences do not broaden automatically: they pause for additional approved input.

`--contribution-interval 6` now sets minimum playback spacing using the nominal scan cadence, rather than skipping matching encounters. `--search-budget-seconds 300` is a soft search-playback target. Unmatched encounters accelerate, with a minimum of one encoded frame each; contribution spacing and final transfer landing can extend the video. No sources are dropped or reordered. Presentation metadata records every encounter hold, launch, arrival and phase count.

Presentation schema 3.0 includes search fade-out, reconstruction fade-in and a final `https://desaparecidos.uy` fade-in/hold/fade-out after the title. Website hold uses `text_hold_seconds`. Comma-separated catalogue names become given names followed by surnames on the person card. These closing changes apply to the shared compositor across all three works.
