...
Traversal `region_search[target_id]` also records `reconstruction_similarity`, `encounter_count` and `stop_reason`. Completion mode uses `reconstruction-complete` only at full pixel coverage, with region acceptance thresholds unchanged. Explicit incomplete studies retain `quality-target-reached` / `approved-frames-exhausted`. Earlier calibration sidecars may contain `contribution-spacing` skips; current matching never drops useful frames for pacing. Similarity remains the lower of whole-image structural correlation and `1 - MAE / 255`, and is reported independently of completion.

`require_complete`, `max_search_batches` and `search_budget_seconds` are recorded settings. `continuation_routes` records acquisition-batch origins, also retained on appended frames in the hashed traversal manifest. Paused searches write `completion_search`, including settings, target-manifest path and per-target decisions, in that ignored manifest, not a final output sidecar/video. Presentation metadata includes `encounter_holds_by_target`, the soft playback budget and its policy; these holds are authoritative when unmatched encounters are accelerated.

A retimed review edition may record `retimed_from` with its parent sidecar path and SHA-256 plus `reconstruction_pixels_unchanged`. Its own runtime provenance, presentation timing and media paths describe the new encoding; its source placements remain bound to the parent reconstruction.
