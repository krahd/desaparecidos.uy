...
The evaluator canonicalises and hashes the complete target-to-history mapping. It records history and placement counts, violations, target-level reasons and the derived `future_source_frames_used` value. Rendering fails before media finalisation when any placement uses a source before encounter, references an unknown source, or has an invalid encounter index.
