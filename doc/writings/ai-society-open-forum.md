# Against Restoration: Responsible AI, Disappearance, and Computational Memorial Systems

## Abstract

This Open Forum paper examines a developing computational memorial artwork about detained-disappeared persons connected to Uruguay's civic-military dictatorship and the wider Southern Cone apparatus of repression. The project reassembles public images of disappeared persons from fragments of the country that survived them: contemporary people, places, streets, walls, landscapes, and visual infrastructures. It is conceived as a triptych: **Todos somos familiares**, which works with fragments from contemporary public images of people; **Están en todas partes**, which works with images of places, surfaces, streets, buildings, and landscapes; and **Seguimos buscando**, which turns traversal and search across the country's visual field into the temporal structure of the artwork.

The paper argues that responsible AI in art and archival practice cannot be reduced to consent, transparency, provenance, or technical safeguards, although all of these remain necessary. In contexts of unresolved state violence, responsible practice also requires a politics of refusal: refusal of forensic authority, refusal of resurrection media, refusal of biometric identification, refusal of archival substitution, and refusal of the fantasy that computation can repair what politics destroyed. The current prototype makes this refusal operational through deliberately impoverished computational means: non-overlapping 24-pixel tiles, a hand-designed six-dimensional colour/contrast/edge descriptor, deterministic L2 nearest-neighbour matching, fragment reuse limits, and an active per-source contribution cap. Its technical crudeness is not a sign of immaturity but part of the ethics of the work: the system is made unable to smooth itself into photorealistic restoration. The project therefore proposes a framework for computational memorial systems based on five principles: archival humility, fragmentary reappearance, non-identification, provenance accountability, and designed incompletion. Rather than presenting AI as a restorative technology, the work treats computation as a means of staging a visual and political condition: disappearance is not an absence sealed in the past, but a structure distributed through the present.
...
This paper presents a developing computational memorial artwork series concerning detained-disappeared persons connected to Uruguay. The project begins from a simple but difficult proposition: the public image of each disappeared person is reassembled from fragments of the country that survived them. These fragments may come from images of contemporary Uruguayans, from streets and surfaces, from landscapes and public buildings, or from traversal across the country's present visual field. The work does not claim to restore the disappeared. It does not claim to reconstruct unknown facts, recover lost bodies, generate testimony, or produce forensic evidence. Its aim is more limited and, for that reason, more precise: to construct a visual condition in which the unresolved persistence of disappearance becomes newly perceptible.

The article treats this developing artwork as a methodological site for thinking responsible AI in relation to memorial practice, archival incompletion, and political violence. It is not a completed reception study or a finished installation analysis. It is a practice-based position paper grounded in an implemented Stage 1 prototype, **Están en todas partes**, the place-fragment work. The current local codebase includes manifest-driven ingestion, target preprocessing, review gates, bounded crawling, duplicate detection, OpenCV/NumPy image gating, deterministic fragment matching, still outputs, optional process videos, and JSON sidecars. The prototype therefore allows the argument to move beyond abstract ethics: the responsible-AI framework is instantiated in technical constraints, file structures, review gates, and refusals built into the workflow.
...
The first lineage is the public visual activism of the Southern Cone. The *Siluetazo*, first carried out in Buenos Aires in 1983 in connection with human-rights organisations including the Mothers of Plaza de Mayo, produced life-size paper silhouettes of absent bodies and installed them in public space. Its force lay in the relation between repetition, anonymity, bodily scale, and urban inscription. The absent body was not restored; it was marked as absent. This distinction is crucial for the present project: making absence visible is not the same as completing it.
...
The project uses AI in a deliberately restricted sense: not as autonomous image synthesis, but as a set of computer-vision, filtering, matching, metadata, and assembly procedures embedded within a governed artistic workflow. The Stage 1 prototype does not depend on a large generative model or on synthetic portrait generation. Its present AI/computational components are narrower and more accountable: local computer-vision gating, deduplication, manifest-driven provenance, deterministic fragment matching, and process videos that expose rather than hide selection and assembly. The work is therefore an AI-and-computation artwork in the broad sense invited by the special collection, not a demonstration of foundation-model image synthesis.
...
The reconstruction algorithm is deliberately modest. Approved source images are decomposed into non-overlapping square fragments, 24 pixels by default, with at most 240 extracted fragments per source image in the current default configuration. Each target tile and source fragment is represented by a six-dimensional, hand-designed descriptor: mean red, green, and blue values; luminance contrast; horizontal edge energy; and vertical edge energy. Matching is deterministic L2 nearest-neighbour search over these descriptors. The running Stage 1 system does not use CLIP, face embeddings, learned perceptual metrics, semantic segmentation, diffusion models, or any learned image embedding for matching. Such techniques could be explored later as clearly marked extensions, but they are not part of the current system and should not be credited with its present ethical or visual behaviour.

The crudeness is part of the ethics. A six-dimensional colour/contrast/edge descriptor and 24-pixel non-overlapping tiles cannot silently produce a persuasive photorealistic restoration. They preserve discontinuity, scale mismatch, colour roughness, and visible joins. The system therefore operationalises designed incompletion at the level of the matcher itself. The work does not merely promise that the artist will later avoid smoothing, beautifying, or restoring the face. The present algorithm makes smooth restoration structurally unavailable. The resulting images may resemble photomosaics, but that resemblance should not be treated as a failure to overcome. The mosaic-like coarseness is the material form of refusal.

The running generation path also enforces a per-source contribution cap. Legacy configurations once allowed `max_contribution_per_source = 0`, meaning that a single source image could contribute without a source-level tile limit and would be constrained only by the per-fragment `reuse_limit`. That was inconsistent with the paper's ethical claim that no single source should dominate the image. The current GUI/API/CLI generation path now normalises unset or zero contribution-cap values to an active default cap of 240 output tiles per source image. The per-fragment `reuse_limit` remains 8, and infeasible caps fail before generation. The dispersed visual field is therefore not only an interpretive aspiration: it is partly embodied in executable constraints.
...
5. **Designed incompletion.** The artwork should preserve seams, gaps, mismatches, fragment boundaries, and temporal dissolution. Incompletion is not a failure to be solved after more technical work. It is a responsible form for a subject that cannot be repaired by representation. In the current Stage 1 prototype, this principle is enforced technically by the restricted descriptor, coarse non-overlapping tiles, deterministic fragment placement, and explicit contribution limits.
...
The first evaluation layer is internal artistic and technical review. Current internal review examines outputs for recognisability of the disappeared person, visibility of the fragmentary process, avoidance of photorealistic closure, absence of recognisable living source persons, and correct display of names and provenance.
